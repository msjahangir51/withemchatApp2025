...
function SignupPage() {
  ...
  const handleSignup = (e)=>{
    e.preventDefault()
    if(!FormData.username || !FormData.password || !FormData.fullname)return toast.error("All Fields are required!");
  ...
  return (
    <div className='pt-10 w-full min-h-screen grid md:grid-cols-2  items-center gap-8'>
      <div className='w-full h-full py-10 flex items-center justify-center'>
        <form onSubmit={handleSignup} className='max-w-md w-auto mx-auto p-8 bg-base-300 rounded-sm'>
          ...
          <div className='flex flex-col gap-4'>
            <div className='form-control'>
              <input  onChange={dataSetHandeler} name='fullname' className='input input-bordered' type="text" placeholder='Enter your surname' />
            </div>
            ...
            <div className='form-control relative'>
              ...
              {<button type='button' onClick={()=> setShowPass(!showpass)} className='absolute cursor-pointer right-3 inset-y-0'>
               {showpass? <Eye/>: <EyeOff/>}
              </button>}
            </div>

            <button type='submit' className='btn btn-primary uppercase -tracking-wider' disabled={SignUpLoading}>{SignUpLoading? <Loader className='size-4 animate-spin'/>: ""}Sign in</button>
          </div>


          <p className='text-sm text-center text-base-content mt-4'>Don't have an account? <Link className='link-hover link-primary ml-1' to={"/signin"}>signin</Link></p>
        </form>
      </div>
